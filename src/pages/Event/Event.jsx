  ...
  return (
    <div className={styles.parent}>
      <div className={styles.container}>
        <img className={styles.image} alt="event image" src={`${BASENAME}${event.poster}`}></img>
        <section className={styles.info}>
          ...
          <div className={styles.buttons}>
            <IoShareSocial className={styles.shareButton} aria-label="copy link button" onClick={handleLinkButton} />
            <button className={styles.saveButton} aria-label="save event button" onClick={handleSaveButton}>{buttonSaveName}</button>
          </div>
        </section>
      </div>
      <p className={styles.discription}>{event.description}</p>
      {isLinkPopupOpen && <PopupLink closePopup={closePopup} pathname={pathname} />}
    </div>
  );
};
